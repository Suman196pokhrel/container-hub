...
Item {
  ...
  BoundedProcess {
    id: process
    timeoutMs: 15000
    // Backstop only: the real cap is the producer-side `head -c` in the
    // piped command itself (see ContainerEngine.qml's producerByteCap) —
    // this just needs to be at least as large so it never truncates
    // tighter than that already has.
    maxOutChars: 700000
    maxErrChars: 16384
    onFinished: {
