...
Item {
  ...
  // A byte cap enforced by the producer (docker/podman itself, piped
  // through `head -c`), not just by BoundedProcess after the fact — see
  // engines/*.js's psCommand/logsCommand and BoundedProcess.qml's header
  // comment.
  ...
  function refresh() {
    // enginePath is only set once the resolved binary has passed
    ...
    // file at enginePath could have been replaced since the last check.
    resolver.recheck(function(ok) {
      ...
      if (spec().needsAccessCheck) {
        ...
        psProcess.command = spec().psCommand(root.enginePath, root.producerByteCap)
        psProcess.start()
      }
    })
  }
  ...
  function _handleBinaryChanged() {
    ...
    root.errorMessage = "The " + spec().binaryName + " binary changed since it was last validated; re-checking."
  ...
  function fetchLogs(id) {
    if (root.enginePath === "" || logs.loading || resolver.busy) return
    if (!Shared.isValidContainerId(id)) {
      logs.text = "Could not read logs: invalid container id."
      return
    }
    resolver.recheck(function(ok) {
      if (!ok) {
        logs.text = "Could not read logs: " + spec().binaryName + " binary failed revalidation."
        root._handleBinaryChanged()
        return
      }
      logs.fetch(spec().logsCommand(root.enginePath, id, root.logTailLines, root.producerByteCap), id)
    })
  }
  ...
  function removeContainer(id) {
    ...
    if (actions.running || actions.checking) {
    ...
    // exact full id still exists right before firing rm -f. inspectCommand
    // fetches state alongside identity too (see engines/*.js) — it isn't
    // used to gate the removal, since rm -f is explicitly state-agnostic
    // by design, but it's reconfirmed in the same fresh query regardless.
    actions.checkExists(spec().inspectCommand(root.enginePath, id), id, function(exists) {
      if (!exists) {
        root.actionErrorMessage = "Container is already gone; nothing removed."
        root.refresh()
        return
      }
      root.runAction(spec().actionCommand(root.enginePath, "remove", id), "remove container")
    })
  }
  ...
  function runAction(command, label) {
    ...
    resolver.recheck(function(ok) {
      if (!ok) {
        root.actionErrorMessage = "Could not " + label + ": " + spec().binaryName + " binary failed revalidation."
        root._handleBinaryChanged()
        return
      }
  ...
  // Resolves the engine binary from a fixed list of trusted absolute paths
  // (never PATH/`which`) and validates the whole path chain before first
  // use — see BinaryResolver.qml. Every later invocation revalidates it
  // first (see refresh()/fetchLogs()/runAction() above), since PATH is
  // attacker-influenceable and Docker's daemon is root-equivalent.
  BinaryResolver {
    id: resolver
    candidatePaths: root.spec().binaryCandidates
